...
impl Sender {
    ...
    /// Send raw bytes without any validation/encoding process. The data will directly be
    /// transmitted to the writer with minimal overhead. This may result in invalid message
    /// transmission, possibly causing the remote end to be disconnected with error.
    #[doc(hidden)]
    pub async fn send_raw(&self, bytes: bytes::Bytes) -> Result<(), SendError> {
